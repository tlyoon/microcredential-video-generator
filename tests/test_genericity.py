...
def _make_doc(path, prefix="3"):
    ...
    d.add_paragraph("Energy may be stored in several physical forms. The useful model depends on the system.")
...
def test_scaffold_profile_for_unrelated_topic(tmp_path):
    p = tmp_path / "topic.docx"
    _make_doc(p, "1")
    extraction = extract_docx(p)
    profile = scaffold_profile(extraction, "energy_course", "Energy Course", target_video_minutes=5)
    assert profile["videos"]
    assert all("core_selectors" in v for v in profile["videos"])
    assert profile["course"]["content_selection"]["priority_terms"] == []

    lesson = profile["videos"][0]
    manifest = build_lesson_manifest(extraction, profile["course"], lesson)
    assert manifest["source_core_block_count"] > 0
...
def test_profile_drift_is_error():
    ...
    assert any(i["severity"] == "error" and "zero authoritative source blocks" in i["message"] for i in issues)
